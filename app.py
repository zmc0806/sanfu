...
class AdvancedLSTMPredictor:
    ...
    def create_advanced_features(self, df):
        ...
        st.write(f"特征工程完成，共 {len(df.columns)} 个特征")
    ...
    def prepare_data(self, df):
        ...
        st.write(f"数据形状 - X: {X.shape}, y: {y.shape}")
    ...
    def plot_training_history(self, history):
        ...
        # 3. MSE曲线
        axes[2].plot(history.history['mse'], label='训练MSE')
        axes[2].plot(history.history['val_mse'], label='验证MSE')
        axes[2].set_title('训练与验证MSE')
...
def main():
    ...
    # 上传数据
    st.subheader("1. 数据上传")
    uploaded_file = st.file_uploader("请上传Excel数据文件", type=["xlsx"])
    ...
    if uploaded_file is not None:
        ...
        # 训练参数设置
        st.subheader("4. 模型训练")
        epochs = st.slider("训练轮数 (Epochs)", min_value=50, max_value=200, value=100)
        batch_size = st.slider("批次大小 (Batch Size)", min_value=16, max_value=128, value=32)
        ...
        if st.button("开始训练模型"):
            ...
            # 预测设置
            st.subheader("5. 未来预测")
            days = st.slider("预测未来天数", min_value=1, max_value=30, value=7)
            
            if st.button("生成未来预测"):
                # 预测未来
                future_predictions = predictor.predict_future_advanced(df, feature_cols, days=days)
                
                st.write("\n" + "="*60)
                st.write("未来预测结果（含置信区间）:")
                st.write("="*60)
                st.dataframe(future_predictions)
                
                # 可视化未来预测
                st.subheader("6. 预测可视化")
